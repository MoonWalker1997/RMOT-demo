            ...
            if outputs[0] is not None:
                ...
                if frame_id % 10 == 0:
                    for each in online_targets:
                        if each.track_id in [27]:
                            show_img_patch(each, img_info)
                ...
                for i, each in enumerate(online_targets):
                    if each.track_id in outside_tracks:
                        if iou_similarity[i, list(outside_tracks.keys()).index(each.track_id)] > 0.5:
                ...
                for i, each in enumerate(online_targets):
                    ...
                    if each.score > 0.7 and each.track_id not in ban_list:
                        hist_target = generate_color_hist(give_img_patch(each, img_info))
