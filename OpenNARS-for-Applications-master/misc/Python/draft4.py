            ...
            if outputs[0] is not None:
                ...
                for each in online_targets:
                    ...
                    if track_name(each) in outside_tracks:
                        # quick hog check
                        # 也要包含对iou的check，如果iou为0

                        # iou为0，hog相似，则说明原物体发生了较大的位移，可以用于更新
                        # iou高，hog相似，好
                        # iou低，hog低，原track已丢失
                        # iou高，hog低，occlude

                        try:
                            h1 = generate_hog(each, img_info["raw_img"])
                        except:
                            # 某些框样式非常离谱
                            continue
                        h2 = outside_tracks[track_name(each)].hog
                        if h1.dot(h2) / ((h1.dot(h1) ** 0.5) * (h2.dot(h2) ** 0.5)) > 0.7:
                            # just "trust" the deep learning model
                            outside_tracks[track_name(each)] = copy.deepcopy(each)
                            # EMA
                            outside_tracks[track_name(each)].mean = 0.1 * each.mean + outside_tracks[track_name(each)].mean * 0.9
                            outside_tracks[track_name(each)].hog = h1
                        else:
                ...
                for i, each in enumerate(new_faces):
                    ...
                    for j, each_track in enumerate(outside_tracks):
                        ...
                        Narsese = []
                        Narsese.extend(distance_to_Narsese(each, each_track, iou_similarity[i, j]))
                        Narsese.extend(question_belong_to_Narsese(each, each_track))
                        Narsese.extend(["10\n"])
                        reasoning_outputs = reasoner.AddInput("\n".join(Narsese), False)

                        for each_r in reasoning_outputs["answers"]:
                            if "belong" in each_r["term"]:
                                f = float(each_r["truth"]["frequency"])
                                if f > 0.8:
                                    # consistent with an old one
                                    tmp = outside_tracks[each_track]
                                    updates.update({each_track: copy.deepcopy(each)})
                                    # EMA
                                    updates[each_track].mean = 0.9 * tmp.mean + 0.1 * updates[each_track].mean
                                    updates[each_track].hog = generate_hog(each, img_info["raw_img"])
                                    belong = True
                                    break

                    if not belong:
                        updates.update({track_name(each): copy.deepcopy(each)})
                        updates[track_name(each)].hog = generate_hog(each, img_info["raw_img"])

                for each in updates:
                    if each in outside_tracks:
                        outside_tracks.update({each + "000": updates[each]})
                    else:
