    ...
    while True:  # each frame
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                # Step 4:
                # appearance similarity, just color histogram
                app_similarity = appearance_similarity(online_targets,
                                                       [outside_tracks[each] for each in active_outside_tracks],
                                                       img_info["raw_img"])

                comprehensive_similarity = iou_similarity * app_similarity

                if frame_id == critical_frame:
                    print(1)

                # Step 5:
                # find confident matching
                I = []
                for i in range(len(online_targets)):
                    if i in I:
                        continue
                    for j in range(len(active_outside_tracks)):
                        row = copy.deepcopy(list(comprehensive_similarity[i, :]))
                        row.sort()
                        if comprehensive_similarity[i, j] > 0.5 and \
                                comprehensive_similarity[i, j] == row[-1] and \
                                comprehensive_similarity[i, j] > 1.3 * row[-2]:
                            I.append(i)
                            break
                ...
                for i in range(len(online_targets)):
                    ...
                    for j in range(len(active_outside_tracks)):
                        ...
                        Narsese = []
                        Narsese.extend(distance_to_Narsese(online_targets[i], active_outside_tracks[j],
                                                           iou_similarity[i, j]))
                        Narsese.extend(appearance_model_to_Narsese(online_targets[i], active_outside_tracks[j],
                                                                   app_similarity[i, j]))
                        # Narsese.extend(hog_to_Narsese(online_targets[i], outside_tracks[active_outside_tracks[j]],
                        #                               active_outside_tracks[j], img_info["raw_img"]))
                        Narsese.extend(question_belong_to_Narsese(online_targets[i], active_outside_tracks[j]))
                        Narsese.extend(question_occluded_to_Narsese(active_outside_tracks[j]))
                        Narsese.extend(["1\n"])
                        ...
                        for each in reasoning_outputs["answers"]:
                            if "belong" in each["term"]:
                                belonging.append(each["term"] + "|" + each["truth"]["frequency"])
                                ...
                                comprehensive_similarity[online_target_idx, outside_track_idx] = float(
                                    each["truth"]["frequency"])
                            elif "occluded" in each["term"]:
                                occlusion.append(each["term"])

                print("frame ID: ", frame_id,
                      ", time used: ", time.time() - time1)
                ...
                tmp = [each.track_id for each in online_targets]
                belonging_dict = {}
                for each in belonging:
                    term, f = each.split("|")
                    f = float(f)
                    A, B = term.split("*")
                    A = int(A[3:-1])
                    B = B.split(")")[0][1:]
                    if f > 0.4:
                        if A in belonging_dict:
                            belonging_dict[A].append([B, f])
                        else:
                            belonging_dict.update({A: [[B, f]]})

                for each in belonging_dict:
                    if len(belonging_dict[each]) > 1:
                        belonging_dict[each].sort(key=lambda x: x[1])
                    t = np.array([tmp.index(each), active_outside_tracks.index(belonging_dict[each][-1][0])])
                    if t not in matches:
                        matches = np.vstack([matches, each])
                    if tmp.index(each) in unmatched2_online_targets:
                        tt = list(unmatched2_online_targets)
                        tt.remove(tmp.index(each))
                        unmatched2_online_targets = np.array(tt)
