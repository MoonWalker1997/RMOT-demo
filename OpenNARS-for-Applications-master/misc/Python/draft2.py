...
def appearance_similarity_to_Narsese(online_target, box_id, outside_track, img):
    return "<(*, T" + str(box_id) + ", " + track_name(online_target) + ") --> similar>. % " + str(
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    reasoner.AddInput("<<(*, $x, $y) --> similar> ==> <(*, $x, $y) --> belong>>.", False)
    reasoner.AddInput("<<(*, $x, $y) --> close> ==> <(*, $x, $y) --> belong>>.", False)
    # ...
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                for each_unmatched_online_target in unmatched_online_targets:  # idx, e.g., 0
                    box_id += 1
                    box_id_ref.update({box_id: each_unmatched_online_target})
                    ...
                    for each_unmatched_outside_track in unmatched_outside_tracks:  # name, e.g., OT1
                        judgments.extend(comparison_to_Narsese(online_targets[each_unmatched_online_target], box_id,
                        ...
                        # num_cycles
                        output_util(tasks_all_cycles, reasoner.AddInput("10", False))

                        for each_answer in tasks_all_cycles["answers"]:
                            if each_answer["term"] != "None":
                                tmp = each_answer["term"].split("-->")[0].split(" * ")
                                box_idx, outside_track_id = int(tmp[0][5:]), tmp[1][:3]
                                if box_id_ref[box_idx] in unmatched_online_targets and \
                                        outside_track_id in unmatched_outside_tracks:  # avoid old answers
                                    similarity_reasoning[
                                        unmatched_online_targets.index(box_id_ref[box_idx]),
                                        unmatched_outside_tracks.index(outside_track_id)] = \
                                        float(each_answer["truth"]["frequency"])
