            ...
            if outputs[0] is not None:
                ...
                for i, each in enumerate(online_targets):
                    ...
                    if each.track_id in outside_tracks:
                        ...
                        if iou_similarity[i, list(outside_tracks.keys()).index(each.track_id)] > \
                            ...
                            # The appearance will be updated when this appearance is "pure" (with no overlapping with
                            # # other detections).
                            ...
                            if inter_iou_similarity[i, i] == inter_iou_similarity[i, :].sum():
                ...
                for i, each in enumerate(online_targets):
                    ...
                    for j, each_id in enumerate(outside_tracks):
                        # at least the iou similarity should be large enough
                        if iou_similarity[i, j] > iou_similarity_thresh_GLC:

                            # if two bboxes are similar (in appearance) enough, then the online target can be used
                            # to update the outside track

                            if compare_color_hist(hist_target, outside_tracks[each_id].appearances) > \
                ...
                for i, each in enumerate(online_targets):
                    ...
                    if each.score > box_score_thresh and each.track_id not in ban_list:
                        hist_target = generate_color_hist(give_img_patch(each, img_info))
                        outside_tracks[each.track_id] = outside_track(each, hist_target)
                ...
                for each in outside_tracks:
                    ...
                    outside_tracks[each].active2 = False
