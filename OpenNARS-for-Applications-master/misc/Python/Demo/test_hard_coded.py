...
appearance_similarity_thresh = 0.5  # [0, 1]
...
class outside_track:
    ...
    def __init__(self, target, appearances = None, appearance_purity = 0):
        ...
        self.appearance_purity = appearance_purity
        self.touched = True  # whether it is just updated
        self.touched_appearance = True  # whether the appearance is just updated
    ...
    def update(self, new_track, appearances = None, appearance_purity = 0):
        """
        ...
        # else:
        #     # mean[4], mean[5] = mean[4] * 0.9, mean[5] * 0.9
        #     mean = self.mean * 0.9 + mean * 0.1
        self.mean = mean
        self.cov = covariance
...
def proportion_private(target, online_targets, inter_iou_similarity, I, img_info):
    # find those bboxes overlapped
    J = []
    ...
    area = target.tlwh[2] * target.tlwh[3] + 1

    mask = np.zeros([img_info["raw_img"].shape[0], img_info["raw_img"].shape[1]])

    mask[min(img_info["raw_img"].shape[0], max(0, int(target.tlbr[1]))):
         min(img_info["raw_img"].shape[0], max(0, int(target.tlbr[3]))),
    min(img_info["raw_img"].shape[1], max(0, int(target.tlbr[0]))):
    min(img_info["raw_img"].shape[1], max(0, int(target.tlbr[2])))] = 1

    for j in J:
        mask[min(img_info["raw_img"].shape[0], max(0, int(online_targets[j].tlbr[1]))):
             min(img_info["raw_img"].shape[0], max(0, int(online_targets[j].tlbr[3]))),
        min(img_info["raw_img"].shape[1], max(0, int(online_targets[j].tlbr[0]))):
        min(img_info["raw_img"].shape[1], max(0, int(online_targets[j].tlbr[2])))] = 0
