...
def print_out(type: PrintType, content, p: float = None, d: float = None, q: float = None, comment_title: str = None,
              end: str = None):
    # beautifully print the results from the reasoner
    if isinstance(p, float) and 0 <= p <= 1:
        ...
        p = '    '
        bg1 = ''
    if isinstance(d, float) and 0 <= d <= 1:
                ...
                for each_unmatched_online_targets in unmatched_online_targets:  # idx, e.g., 0
                    ...
                    for each_unmatched_outside_track in unmatched_outside_tracks:  # name, e.g., OT1
                        ...
                        for each_Narsese in judgments + [question]:
                            reasoner.input_narsese(each_Narsese, go_cycle=False)
                        tasks_all_cycles = []
                        for _ in range(10):  # num_cycles
                            tasks_all = reasoner.cycle()
                            tasks_all_cycles.append(deepcopy(tasks_all))

                        for tasks in tasks_all_cycles:
                            _, _, _, answers_question, _, _ = tasks
